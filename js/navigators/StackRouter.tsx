...
import BaseRouter from "@react-navigation/routers/src/BaseRouter";
import type {
...
export default function StackRouter(options: StackRouterOptions) {
  const router: Router<StackNavigationState<ParamListBase>, CommonNavigationAction | StackActionType> = {
    ...BaseRouter,
    ...
    getStateForAction(state, action, options) {
      ...
      console.log("🚀🚀🚀wimi======>>>state", state);
      console.log("🚀🚀🚀wimi======>>>action", action);
      console.log("🚀🚀🚀wimi======>>>options", options);
      switch (action.type) {
        case "REPLACE": {
          const index =
        ...
        case "PUSH":
          if (state.routeNames.includes(action.payload.name)) {
            ...
            const route = id
              ? state.routes.find(
                  route => route.name === action.payload.name && id === getId?.({ params: route.params })
                )
              : undefined;
        ...
        case "NAVIGATE":
          ...
          if (action.payload.key || action.payload.name) {
            ...
            const getId =
              ...
              action.payload.key === undefined && action.payload.name !== undefined
            ...
            if (id) {
              ...
              (state.routes[state.index].name === action.payload.name && action.payload.key === undefined) ||
              ...
              for (let i = state.routes.length - 1; i >= 0; i--) {
                if (
                  (state.routes[i].name === action.payload.name && action.payload.key === undefined) ||
